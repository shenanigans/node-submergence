...
/**     @submodule/class submergence:Context
    When a template is executed, the context is seeded with several properties. These properties are
    overriden by the [configuration](submergence:Action:Configuration#context) and [reply content]
    (submergence:Reply#content) of the Action.
...
@member/String|undefined ServerError
    When the server throws `403` or `502` within the [Transport](.) layer, a short message is
    provided.
*/


/**     @module/class submergence:Transport
    Manages client connections over REST and Socket.io. Parses credentials and validates through
    [Authentication] (submergence.Authentication). Builds [Replies](submergence:Reply) and feeds them
    to [Actions](submergence:Action) acquired from [Router](submergence:Router).
@argument/submergence parent
@argument/submergence:Configuration config
*/
...
/**     @member/Function listen
    Begins listening on the configured port(s) and serving [Actions](submergence:Action). If SSL is
...
Transport.prototype.listen = function (port, router, callback) {
    ...
    this.authentication.init (function(){
        ...
        server.on ('error', function (err) {
            self.parent.logger.fatal ({ port:port, err:err }, 'server failed to listen');
    ...
    this.reaction_REST = function (request, response) {
        ...
        router.getAction (request, path.pathname, function (action, params) {
            ...
            authentication.getSession (requestDomain, path, cookies, function (err, authInfo) {
                ...
                authInfo.useragent = request.headers['user-agent'] || '';
                ...
                var reply = new Reply (function (status, events, content, html) {
                    ...
                    // "submergenceEvents" boilerplate injects events into template context
                    if (events.length) {
                        var eventScript =
                            '<script type="text/javascript">(function(){'
                          + 'var submergence=require("submergence");submergence.sendEvents('
                ...
                request.on ('end', function(){
                    ...
                    if (bodyChunks.length) {
                        try {
                            actionRequest.body = JSON.parse (Buffer.concat (bodyChunks).toString());
                        } catch (err) {
                            station.logger.warn ('rejected malformed request');
                            return closeRequest (400, { ClientError:'malformed request' });
                        }
                    }
                    action.run (station, authInfo, actionRequest, reply);
    ...
    */
    this.handshake_SocketIO = function (socket, callback) {
        ...
        authentication.getSession (socket.requestDomain, path, cookies, function (err, agent) {
            ...
            agent.useragent = socket.request.headers['user-agent'] || '';
            socket.agent = agent;
        ...
        socket.on ('action', function (actionDoc) {
            ...
            if (actionDoc.method == 'OPTIONS') {
                station.router.getOptions (actionDoc, function (options) {
                    var response = { status:200, path:socket.path, content:options };
                    if (actionDoc._id)
