...
Transport.prototype.createReactions = function (router) {
    ...
    */
    this.reaction_REST = function (request, response) {
        ...
        router.getAction (request, function (action, params) {
            if (!action) {
            ...
            authentication.getSession (path, cookies, function (err, authInfo) {
                ...
                var actionRequest = {
                    ...
                    query:      queryDoc,
                    params:     params || [],
                    domain:     Object.hasOwnProperty.call (request.headers, 'host') ?
    ...
    */
    this.reaction_SocketIO = function (socket, callback) {
        ...
        socket.on ('action', function (actionDoc) {
            ...
            router.getAction (actionDoc, function (action, params) {
                if (!action) {
                    // reply 404
                    station.logger.info ({
                        transport:  'socket.io',
